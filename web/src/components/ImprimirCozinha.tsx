...
export function ImprimirCozinha({ pedidoId, reimpressao = false }: { pedidoId: string; reimpressao?: boolean }) {
  ...
  function imprimirUSB(){
    ...
    setOcupado(true)
    void imprimirUrl(`/api/bar/comprovante?pedido_id=${encodeURIComponent(pedidoId)}&via=cozinha&papel=${config.papel}&reimpressao=${reimpressao?'1':'0'}`).finally(()=>setOcupado(false))
  }
  ...
  return <div className="space-y-2">
    ...
    <div className="flex flex-wrap gap-2"><BotaoImpressao type="button" variant="outline" disabled={ocupado} onClick={imprimirUSB} className="gap-2"><Printer size={16}/>{reimpressao?'Reimprimir pelo navegador':'Imprimir pelo navegador'}</BotaoImpressao>
    <BotaoImpressao variant="outline" disabled={ocupado} onClick={imprimir} className="gap-2"><Printer size={16}/>{ocupado ? 'Enviando...' : reimpressao ? 'Reimprimir pela rede' : 'Enviar pela rede'}</BotaoImpressao></div>
