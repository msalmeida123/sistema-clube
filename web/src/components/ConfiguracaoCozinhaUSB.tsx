...
export default function ConfiguracaoCozinhaUSB(){
 ...
 function testar(){
  void imprimirHtml(gerarComandaCozinha({numero_pedido:0,mesa:'TESTE',cliente_nome:config.nome,created_at:new Date().toISOString(),observacao:'TESTE DE IMPRESSÃO USB — NÃO PREPARAR ALIMENTOS',bar_itens_pedido:[{produto_nome:'Teste de impressão da cozinha',quantidade:1,enviar_cozinha:true}]},config.papel)).catch(()=>{})
 ...
 <div className="flex gap-3"><Button onClick={salvar}>Salvar USB</Button><BotaoImpressao variant="outline" onClick={testar}>Teste USB</BotaoImpressao></div>
