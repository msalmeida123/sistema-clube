...
import {GET,DELETE,PUT} from '@/app/api/configuracoes/fundo-login/route'
import {GET as imagem} from '@/app/api/tema/fundo-login/route'
...
it('rejeita upload de outra origem antes de acessar banco',async()=>{const r=await PUT(new NextRequest('https://clube.test/api/configuracoes/fundo-login',{method:'PUT',headers:{host:'clube.test',origin:'https://outro.test'},body:'x'}));expect(r.status).toBe(403);expect(from).not.toHaveBeenCalled()})
...
it('imagem pública usa apenas o clube resolvido pelo domínio',async()=>{const r=await imagem(new NextRequest('https://clube.test/api/tema/fundo-login?clube_id=outro'));expect(r.status).toBe(204);expect(eq).toHaveBeenCalledWith('clube_id','clube-b')})
it('remove apenas o fundo do clube autenticado',async()=>{eq.mockResolvedValueOnce({error:null});const r=await DELETE(new NextRequest('https://clube.test/api/configuracoes/fundo-login',{method:'DELETE',headers:{host:'clube.test',origin:'https://clube.test'}}));expect(r.status).toBe(200);expect(eq).toHaveBeenCalledWith('clube_id','clube-a')})
