...
test('QR personalizado do dependente é diferente do QR do titular',()=>{
 expect(codigoCarteirinha(id,'SOCIO-'+id)).toBe('SOCIO-'+id)
 expect(codigoCarteirinha(id,'DEP-'+id,true)).toBe('DEP-'+id)
 expect(codigoCarteirinha(id,'SOCIO-'+id)).not.toBe(codigoCarteirinha(id,'DEP-'+id,true))
})
