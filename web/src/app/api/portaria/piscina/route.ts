...
import {buscarPiscina,registrarPiscina,historicoPiscina} from '@/lib/portaria-piscina'
...
export async function GET(req:Request){
 const acesso=await acessoRota('portaria_piscina')
 if(!acesso)return resposta({error:'Sem permissão para a Portaria Piscina.'},403)
 let pagina,limite;try{({pagina,limite}=paginacao(req.url))}catch{return resposta({error:'Paginação inválida.'},400)}
 try{return resposta(await historicoPiscina(servicoAuditado(acesso.user.id),pagina,limite))}catch{return resposta({error:'Não foi possível carregar os acessos da piscina.'},503)}
}
export async function POST(req:NextRequest){
 ...
 try{
  ...
  const result=body.acao==='buscar'?await buscarPiscina(db,body.valor):await registrarPiscina(db,body.associado_id,body.tipo)
