...
export async function GET(){try{
 ...
 const {data,error}=await appDb().from('clube_verso_carteirinha').select('versao').eq('clube_id',ator.clube).maybeSingle()
 if(error)throw error
 return json({versao:data?.versao ?? null})
}catch(e){return erroTema(e)}}
export async function PUT(req:NextRequest){try{
 origemTema(req);const ator=await atorTema();const db=appDb();await limite(db,`verso-carteirinha:${ator.user.id}`,20)
 ...
 const {error}=await db.from('clube_verso_carteirinha').upsert({clube_id:ator.clube,dados,versao,atualizado_por:ator.user.id,atualizado_em:new Date().toISOString()})
 if(error)throw error
 return json({versao})
}catch(e){return erroTema(e)}}
export async function DELETE(req:NextRequest){try{
 origemTema(req);const ator=await atorTema()
 const {error}=await appDb().from('clube_verso_carteirinha').delete().eq('clube_id',ator.clube)
 if(error)throw error
