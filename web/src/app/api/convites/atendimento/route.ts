...
export async function POST(req:NextRequest){
 ...
 const p=await verificarPermissao(auth,user.id,b.acao==='exame'?'exames':'portaria')
 const consultaMedica=b.acao==='consultar'&&(await verificarPermissao(auth,user.id,'exames')).autorizado
 if(!p.autorizado&&!consultaMedica)return json({error:'Sem permissão para esta operação.'},403)
