...
export async function GET(){
 const a=await acessoRota('infracoes')
 if(!a)return NextResponse.json({error:'Sem permissão'},{status:403})
 const banco=servicoAuditado(a.user.id)
 const [lista,pendentes]=await Promise.all([
  banco.from('associado_app_relatos').select('*,associado:associados(nome,numero_titulo)').order('criado_em',{ascending:false}).limit(200),
  banco.from('associado_app_relatos').select('id',{count:'exact',head:true}).eq('status','recebido'),
 ])
 const erro=lista.error||pendentes.error||pendentes.count===null
 return NextResponse.json(erro?{error:'Falha ao carregar relatos'}:{relatos:lista.data,recebidos:pendentes.count},{status:erro?500:200,headers:{'Cache-Control':'no-store'}})
}
