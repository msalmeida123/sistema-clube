...
export type Rubrica={campo:CampoRubrica;codigo:string;descricao:string;referencia:string;valor:number;incidencias?:{inss:boolean;irrf:boolean;fgts:boolean}}
export type DetalhesHolerite={codigo_funcionario:string;sede:string;admissao:string;conta:string;dependentes:number|null;base_inss:number|null;base_irrf:number|null;base_fgts:number|null;salario_contratual:number|null;rubricas:Rubrica[];fgts_aliquota?:number|null;fgts_valor?:number|null;outras_deducoes_irrf?:number;regime_irrf?:'legais'|'mais_favoravel';mes_irrf?:string;calculo_encargos?:unknown}
