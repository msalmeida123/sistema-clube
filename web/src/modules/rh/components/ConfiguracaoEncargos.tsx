...
export function ConfiguracaoEncargos(){
 ...
 return <form onSubmit={salvar} className="space-y-4 rounded-lg border p-5 max-w-4xl">
  ...
  <fieldset disabled={busy||!admin||!!erro} className="space-y-4">
   ...
   {p.inss.map((f,i)=><div key={i} className="grid gap-3 grid-cols-1 sm:grid-cols-2"><label>Faixa {i+1} · até (R$)<Input required type="number" min={0.01} step="0.01" value={f.ate} onChange={e=>setP({...p,inss:p.inss.map((x,j)=>j===i?{...x,ate:Number(e.target.value)}:x)})}/></label><label>Alíquota (%)<Input required type="number" min={0} max={100} step="0.01" value={f.aliquota} onChange={e=>setP({...p,inss:p.inss.map((x,j)=>j===i?{...x,aliquota:Number(e.target.value)}:x)})}/></label></div>)}
