...
export async function hashSenha(senha:string){const salt=randomBytes(16).toString('hex');const key=await scrypt(senha,salt,64) as Buffer;return salt+':'+key.toString('hex')}
export async function confereSenha(senha:string,hash?:string|null){if(!hash)return false;const [salt,h]=hash.split(':');if(!salt||!h)return false;const key=await scrypt(senha,salt,64) as Buffer;const wanted=Buffer.from(h,'hex');return wanted.length===key.length&&timingSafeEqual(wanted,key)}
export async function limite(db:ReturnType<typeof appDb>,key:string,max=10){const {data,error}=await db.rpc('app_associado_limite',{p_chave:digest(key),p_max:max});if(error||data!==true)throw Error('Aguarde alguns minutos antes de tentar novamente.')}
...
export async function sessaoAssociado(req:NextRequest,allowTemporary=false){
 ...
 if(error||!s||s.trocar_senha&&!allowTemporary)return null;
 const {data:a}=await db.from('associados').select('id,nome,numero_titulo,status,qr_code').eq('id',s.associado_id).maybeSingle();
 if(!a)return null;return {db,associado:a,sessao:s,tokenHash};
}
