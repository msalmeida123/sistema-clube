...
export function htmlDocumento(titulo: string, conteudo: string) {
  ...
  </style></head><body><div class="aviso"><button id="imprimir-documento" type="button">Imprimir / Salvar em PDF</button><p>Escolha a impressora de documentos ou “Salvar como PDF”. Papel A4.</p><p id="estado-impressao" role="status"></p></div><article>${conteudo}</article>
  <script>
  document.getElementById('imprimir-documento').addEventListener('click', async function () {
    const botao = this;
    const estado = document.getElementById('estado-impressao');
    botao.disabled = true;
    estado.textContent = 'Preparando imagens para impressão...';
    let prazo;
    try {
      await Promise.race([
        Promise.all([document.fonts.ready, ...Array.from(document.images).map(img => img.decode())]),
        new Promise((_, reject) => { prazo = setTimeout(() => reject(new Error('Tempo esgotado ao carregar as imagens.')), 15000); })
      ]);
      clearTimeout(prazo);
      estado.textContent = 'Selecione a impressora e confirme a impressão. Você pode tentar novamente nesta página.';
      window.focus();
      window.print();
    } catch (_) {
      estado.textContent = 'Não foi possível preparar a impressão. Recarregue o convite pelo sistema e tente novamente.';
    } finally {
      clearTimeout(prazo);
